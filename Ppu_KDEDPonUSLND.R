Ppu_KDEDPonUSLND <- function(data, Residue, USL, BW = "Silver1.06") {
  ...
  library(knitr)
  library(kableExtra)
  residue_col <- deparse(substitute(Residue))
  usl_col <- deparse(substitute(USL))
  
  if (!(residue_col %in% names(data))) stop(paste0("Column '", residue_col, "' not found in the data."))
  if (!(usl_col %in% names(data))) stop(paste0("Column '", usl_col, "' not found in the data."))
  ...
  if (is.character(BW)) {
    ...
    stop("BW must be a numeric bandwidth value > 0 or one of 'Silver1.06', 'Silver0.9', 'Silver0.9IQR'.")
  }
