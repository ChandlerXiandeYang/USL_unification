...
# 2.1 Ppu for one data with USL-normalization
...
```
# 2.2 Ppu and confidence interval with bootstrap for one data  with USL-normalization
...
```
# 3.3 Ppu CQAWWC monitoring model with training, CIL, and monitoring
...
```
# 5.1 Tradition Ppu for each subgroup and the minmal as the CQA's Ppu 
```{r Ppu_SWWC_KDEDP}
#| echo: False
#| message: False
#| warning: False
...
#   data_clean <- data %>%
#     filter(!is.na({{Residue}}), !is.na({{USL}})) %>%
#     select(Residue = {{Residue}}, USL = {{USL}})
#   
...
```
# 5.2 Tradition Ppu for each subgroup and the minmal as the 1 to 3 CQAs' Ppu
...
# 6.1 Ppu of the three subgroups of DAR by USL normalization (they are equal to each subgrop Ppu in the following section)

```{r Ppu_KDEDPonUSLND_122_2}
#| echo: False
#| message: False
#| warning: False
#| tbl-cap: Subgroup of DAR with USL=122.2 ug/swab
#| label: tbl-KDEDPonUSLND122
SWWC122_KDEDPonUSLND <- Ppu_KDEDPonUSLND(data=Eq_DAR_122, Residue=DAR, USL=USL)
colnames(SWWC122_KDEDPonUSLND ) <- paste0("**", colnames(SWWC122_KDEDPonUSLND ), "**")
kable(SWWC122_KDEDPonUSLND )
```
```{r Ppu_KDEDPonUSLND_169_7}
#| echo: False
#| message: False
#| warning: False
#| tbl-cap: Subgroup of DAR with USL=169.7 ug/swab
#| label: tbl-KDEDPonUSLND169
SWWC169_KDEDPonUSLND <- Ppu_KDEDPonUSLND(data=Eq_DAR_169, Residue=DAR, USL=USL)
colnames(SWWC169_KDEDPonUSLND ) <- paste0("**", colnames(SWWC169_KDEDPonUSLND ), "**")
kable(SWWC169_KDEDPonUSLND )
```
```{r Ppu_KDEDPonUSLND_20}
#| echo: False
#| message: False
#| warning: False
#| tbl-cap: Subgroup of DAR with USL=20 ug/swab
#| label: tbl-KDEDPonUSLND20
SWWC20_KDEDPonUSLND <- Ppu_KDEDPonUSLND(data=Eq_DAR_20, Residue=DAR, USL=USL)
colnames(SWWC20_KDEDPonUSLND ) <- paste0("**", colnames(SWWC20_KDEDPonUSLND ), "**")
kable(SWWC20_KDEDPonUSLND )
```
...
# kable(CQAWP_Model)
# kable(CQAWP_Model, 
#        format = "latex", booktabs = TRUE, longtable = TRUE, caption = "Monitoring Results of the Cleaning Process for Equipment A by CQAWWC-BAKDEDPonUSLND Model", row.names = FALSE) %>%
#   kable_styling(font_size = 9, position = "center", latex_options = c("scale_down", "striped", "hold_position"))


library(knitr)
library(kableExtra)

# Bold column names for LaTeX
colnames(CQAWP_Model) <- paste0("\\textbf{", colnames(CQAWP_Model), "}")

kable(CQAWP_Model, 
      format = "latex", booktabs = TRUE, longtable = TRUE, 
      caption = "Monitoring Results of the Cleaning Process for Equipment A by CQAWWC-BAKDEDPonUSLND Model", 
      row.names = FALSE) %>%
  kable_styling(font_size = 9, position = "center", 
                latex_options = c("scale_down", "striped", "hold_position")) %>%
  column_spec(1, bold = TRUE, border_right = TRUE, color = "black", background = "grey")
